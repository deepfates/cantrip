...
`Cantrip.Medium.Code.PortChild.do_write_frame/2` now catches write failures, including `:epipe`, and returns `{:error, reason}` instead of letting the linked protocol process crash. The top-level child loop checks reply-write results; a failed write logs a stderr Goodbye line and exits normally.
...
The regression test `port child treats epipe during reply as graceful shutdown` uses the real child process and packet protocol, closes the parent port before the delayed child reply, asserts the Goodbye log, and asserts no new `erl_crash.dump*` appears in the repo root.

## Verification

- `mix test test/port_code_medium_test.exs` passed: 21 tests, 0 failures.
- `mix test --only mnesia --max-cases 1` passed: 56 tests, 0 failures.
