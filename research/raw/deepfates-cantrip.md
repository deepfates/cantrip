---
title: "GitHub - deepfates/cantrip: agent grimoire starter pack"
...
[](https://github.com/deepfates/cantrip#-cantrip)
A template for building your own agents. Clone it, learn from it, make it yours.

What is an agent?
-----------------

[](https://github.com/deepfates/cantrip#what-is-an-agent)
...
[](https://github.com/deepfates/cantrip#how-does-it-know-when-to-stop)
...
[](https://github.com/deepfates/cantrip#what-are-tools)
...
[](https://github.com/deepfates/cantrip#get-started)
...
[](https://github.com/deepfates/cantrip#learn-by-example)
The examples build on each other. Work through them in order.

### The basics

[](https://github.com/deepfates/cantrip#the-basics)
**[`01_core_loop.ts`](https://github.com/deepfates/cantrip/blob/main/examples/01_core_loop.ts)** — The loop with a fake LLM that returns hardcoded responses. No API keys needed. Start here to see how the pieces fit together.

**[`02_quick_start.ts`](https://github.com/deepfates/cantrip/blob/main/examples/02_quick_start.ts)** — A real agent using Claude. Has an `add` tool and a `done` tool. Your first working agent.

**[`03_providers.ts`](https://github.com/deepfates/cantrip/blob/main/examples/03_providers.ts)** — Shows how to swap between Anthropic, OpenAI, Google, OpenRouter, and local models.

**[`04_dependency_injection.ts`](https://github.com/deepfates/cantrip/blob/main/examples/04_dependency_injection.ts)** — How to give tools access to databases, API clients, or test mocks.

### Builtin tool modules

[](https://github.com/deepfates/cantrip#builtin-tool-modules)
**[`05_fs_agent.ts`](https://github.com/deepfates/cantrip/blob/main/examples/05_fs_agent.ts)** — A coding agent with sandboxed filesystem tools (`read`, `write`, `edit`, `glob`, `bash`).

**[`06_js_agent.ts`](https://github.com/deepfates/cantrip/blob/main/examples/06_js_agent.ts)** — A computational agent with two JavaScript tools: `js` (persistent REPL, no I/O) and `js_run` (fresh sandbox with fetch and virtual fs).

**[`07_browser_agent.ts`](https://github.com/deepfates/cantrip/blob/main/examples/07_browser_agent.ts)** — A web browsing agent with a persistent headless browser (via Taiko).

### Putting it together

[](https://github.com/deepfates/cantrip#putting-it-together)
**[`08_full_agent.ts`](https://github.com/deepfates/cantrip/blob/main/examples/08_full_agent.ts)** — Combines filesystem, JavaScript, and browser tools into one agent. Use this as a starting point for your own agent.

### Advanced patterns

[](https://github.com/deepfates/cantrip#advanced-patterns)
**[`09_rlm.ts`](https://github.com/deepfates/cantrip/blob/main/examples/09_rlm.ts)** — Recursive Language Model. Handle massive contexts (10M+ tokens) by keeping data in a sandbox instead of the prompt. The LLM writes code to explore it and can spawn sub-agents to analyze chunks.

**[`10_rlm_chat.ts`](https://github.com/deepfates/cantrip/blob/main/examples/10_rlm_chat.ts)** — Interactive RLM REPL. Load a file as context and query it conversationally.

**[`11_rlm_memory.ts`](https://github.com/deepfates/cantrip/blob/main/examples/11_rlm_memory.ts)** — RLM with auto-managed conversation history. Older turns slide into searchable context while keeping the active prompt window small.

**[`12_acp_agent.ts`](https://github.com/deepfates/cantrip/blob/main/examples/12_acp_agent.ts)** — Basic agent served over [Agent Client Protocol](https://github.com/agentclientprotocol/spec). Connect from any ACP-compatible editor (VS Code, Claude Desktop, etc.).

**[`13_acp_rlm_memory.ts`](https://github.com/deepfates/cantrip/blob/main/examples/13_acp_rlm_memory.ts)** — RLM memory agent over ACP. Combines sliding window memory management with editor integration.

**[`14_rlm_browser.ts`](https://github.com/deepfates/cantrip/blob/main/examples/14_rlm_browser.ts)** — RLM with browser automation. Interactive REPL where the agent can browse the web and delegate to sub-agents.

**[`15_acp_rlm_browser.ts`](https://github.com/deepfates/cantrip/blob/main/examples/15_acp_rlm_browser.ts)** — RLM browser agent over ACP. The most powerful setup: browser automation, sub-agent delegation, optional memory management, all accessible from your editor. Use `--headed` for visible browser, `--memory N` for sliding window.

Included Tools Library
----------------------

[](https://github.com/deepfates/cantrip#included-tools-library)
...
[](https://github.com/deepfates/cantrip#optional-features)
...
[](https://github.com/deepfates/cantrip#providers)
...
[](https://github.com/deepfates/cantrip#agent-client-protocol-acp)
...
[](https://github.com/deepfates/cantrip#quick-setup)
...
[](https://github.com/deepfates/cantrip#the-philosophy)
...
[](https://github.com/deepfates/cantrip#make-it-yours)
...
[](https://github.com/deepfates/cantrip#license)
MIT
