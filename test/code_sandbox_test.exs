defmodule Cantrip.CodeSandboxTest do
  use ExUnit.Case, async: false

  alias Cantrip.FakeLLM

  defp code_cantrip(llm, opts \\ []) do
    wards = Keyword.get(opts, :wards, [%{max_turns: 10}, %{sandbox: :unrestricted}])
  ...
  describe "code eval sandbox" do
    test "eval timeout does not hang the entity" do
      ...
      {:ok, cantrip} =
        code_cantrip(llm,
          wards: [%{max_turns: 10}, %{sandbox: :unrestricted}, %{code_eval_timeout_ms: 50}]
        )
