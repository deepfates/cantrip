defmodule Cantrip.CircleRuntimeTest do
  ...
  test "CIRCLE-10 injected gate dependencies are used" do
    ...
    llm =
      {FakeLLM,
       FakeLLM.new([
         %{tool_calls: [%{gate: "read_file", args: %{path: "test.txt"}}]},
         %{tool_calls: [%{gate: "done", args: %{answer: "ok"}}]}
       ])}
    ...
    {:ok, cantrip} =
      Cantrip.new(
        ...
        circle: %{
          type: :conversation,
          gates: [%{name: :done}, %{name: :read_file, dependencies: %{root: root}}],
          wards: [%{max_turns: 10}]
        }
      )
  ...
  test "CIRCLE-10 code medium exposes non-reserved gates as host functions" do
    ...
    llm =
      {FakeLLM,
       FakeLLM.new([
         %{code: "text = read_file.(%{path: \"snippet.txt\"})\ndone.(\"read:\" <> text)"}
       ])}
