...
- **The Mirror of Language** (deepfates) — Naming is operative, not decorative. Five magic categories. The simulator hypothesis recast as llm-as-stateless-function: the model doesn't persist, only the pattern does. (naming, circle)
...
- **Magic Crystals** (deepfates) — Origin of "llm" as the term for the LLM: crystallized intelligence from weeks of GPU operation. A llm is a frozen pattern, not a living thing. (naming)
...
- **Building Effective Agents** (Anthropic) — Orchestrator-workers = call_entity, ACI = tool_view(). [anthropic.com](https://www.anthropic.com/engineering/building-effective-agents) (recursion, RL)
...
- **Ecology of Intelligences** (deepfates) — model size hierarchy validates child llm may differ from parent (recursion)
