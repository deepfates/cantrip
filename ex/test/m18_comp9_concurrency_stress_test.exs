defmodule CantripM18Comp9ConcurrencyStressTest do
  ...
  test "COMP-9 preserves multiple concurrent child subtrees with parent_terminated truncation" do
    parent_code = """
    c1 = CantripM18Comp9ConcurrencyStressTest.slow_child_llm("A")
    c2 = CantripM18Comp9ConcurrencyStressTest.slow_child_llm("B")
    c3 = CantripM18Comp9ConcurrencyStressTest.slow_child_llm("C")
    _ = call_entity_batch.([
      %{intent: "c1", llm: c1},
      %{intent: "c2", llm: c2},
      %{intent: "c3", llm: c3}
  ...
  def slow_child_llm(label) do
    ...
    {FakeLLM, FakeLLM.new(slow_turns ++ [%{code: done_code}])}
  end
end
