    ...
    (when (or (= medium :code) (= medium :minecraft))
      (let [gate-names (gates/gate-names (:gates circle))
        ...
        (str "You write " medium-name " code that executes in a SCI (Small Clojure Interpreter) sandbox.\n"
             ...
             "### SANDBOX PHYSICS\n"
             "1. Host functions (call-gate, submit-answer) emit calls that execute AFTER your code returns.\n"
             "   They do NOT return results inline. To react to a gate result, read it in the next turn.\n"
             "2. Variables defined with (def ...) persist across turns.\n"
             "3. Standard Clojure core is available. No require/ns/eval/slurp/spit.\n"
             "4. Do NOT use try/catch around host functions — they do not throw.\n\n"
             "### HOST FUNCTIONS\n"
             gate-lines "\n\n"
